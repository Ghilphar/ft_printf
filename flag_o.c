...
/*   Updated: 2019/01/20 23:21:23 by fgaribot         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
va_list *flag_o(va_list ap, t_data **data)
{
	...
	if ((*data)->castl == 1)
	{
		print_unsigned_base((unsigned long)va_arg(ap, unsigned int), "01234567",
				&data);
		(*data)->castl = 0;
		return (0);
	}
	if ((*data)->castl == 2)
	{
		print_unsigned_base((unsigned long long)va_arg(ap, unsigned int), "01234567",
				&data);
		(*data)->castl = 0;
		return (0);
	}
