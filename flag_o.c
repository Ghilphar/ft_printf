...
/*   Updated: 2019/02/08 14:25:56 by fgaribot         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
va_list *flag_o(va_list ap, t_data **data)
{
	...
	{
		if ((*data)->castl == 1)
			printf_signed_base((unsigned long)va_arg(ap,unsigned long long),
					"01234567", &data);
		if ((*data)->castl == 2)
			printf_signed_base((unsigned long long)va_arg(ap, unsigned long long), "01234567", &data);
		return (0);
	}
	/*
	   if ((*data)->casth == 1)
